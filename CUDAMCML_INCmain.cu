...
//
//			
//			CUDA-based Monte Carlo simulation of photon migration in layered media with inclusions (CUDAMCML_INC) is based in CUDA-based Monte Carlo simulation of photon migration in layered media (CUDAMCML).
//	
//			All comments concerning with the original version (CUDAMCML) are found in the following:
// 
...
__device__ __constant__ unsigned int n_inclusions_dc[1];	
__device__ __constant__ float fx_dc[1];
__device__ __constant__ float fy_dc[1];
__device__ __constant__ LayerStruct layers_dc[MAX_LAYERS];	
__device__ __constant__ DetStruct det_dc[1];
__device__ __constant__ IncStruct inclusion_dc[MAX_INCS];
__device__ __constant__ unsigned int ignoreAdetection_dc[1];

#include "CUDAMCML_INCmem.cu"
#include "CUDAMCML_INCio.cu"
#include "CUDAMCML_INCrng.cu"
#include "CUDAMCML_INCtransport.cu"
